...
def gps_sigmas(m: VrsGpsMeasurement) -> tuple[float, float]:
    ...
    print(f"GPS measurement at {m.easting}, {m.northing} has lat_std {m.lat_std}, lon_std {m.lon_std}")
    ...
    for sensor_name, measurement in dataset.iter_synchronized(['encoder', 'vrs']):
        ...
        if sensor_name == 'vrs':
            ...
            if not initialized:
                ...
                graph_manager.add_variable(graph_manager.pose_key(pose_index), curr_pose)
                initialized = True
                print(f"Added prior factor for initial pose {measurement.easting}, {measurement.northing}")


        # Add an initial guess for the new pose variable
        if not initialized:
        ...
        if sensor_name == 'encoder':
            if prev_encoder is not None:
                ...
                prev_pose = graph_manager.current_estimate.atPose2(prev_key) if graph_manager.current_estimate is not None else origin
        ...
        if step % 50 == 0:
            ...
            current_pose = graph_manager.current_estimate.atPose2(graph_manager.pose_key(pose_index))
            print(f"Current estimate for pose {pose_index}: {current_pose}")
            print(f"x: {current_pose.easting()}, y: {current_pose.northing()}, theta: {current_pose.theta()}")
...
if __name__ == "__main__":
    ...
    dataset_path = Path(file_path / '..' / 'ortelius' / 'datasets' / 'data' / 'urban33-yeouido')
